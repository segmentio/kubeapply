...
Thanks for using `kubeapply` over the years. The Segment Developer Platform team has decided to archive this repository, since we've moved on to different ways of deploying our Kubernetes applications. Specifically, we now use a GitOps process where we store manifests in repositories, generated by tools like [Helm](https://helm.sh), and then use [Argo CD](https://argo-cd.readthedocs.io/en/stable/) to apply them.

This repository will remain available for current users and anyone who can make use of what it contains.
